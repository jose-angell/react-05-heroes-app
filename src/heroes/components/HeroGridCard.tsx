...
export const HeroGridCard = ({hero}:Props) => {
    ...
    <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
        <div className="relative h-64 ">
            ...
            {
                (hero.universe === 'DC') ? (
                    <Badge className="absolute top-3 right-3 text-xs bg-blue-800 text-white">DC</Badge>
                ) : (
            ...
            onClick={() => toggleFavorite(hero)}>
                <Heart className={`h-4 w-4 ${
                    isFavorite(hero) ? 'text-red-500' : 'text-gray-400'
                }`} />
            </Button>
        ...
        <CardContent className="space-y-4">
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">Primera aparicion{hero.firstAppearance}</div>
        </CardContent>
    </Card>
)}
